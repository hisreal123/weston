...
const PropertySection: React.FC = () => {
  ...
  const [defaultCat, setDefaultCat] = useState<PropertyData | null>(null);
  ...
  // setting a default category display
  useEffect(() => {
    const fetchCatData = async () => {
      try {
        const res = await axiosInstance.get(
          `/api/properties/categories/Living%20House`
        );
        const data: PropertyData = res.data;
        setDefaultCat(data);
        // console.log(data);
        ...
        // Store images in localStorage
        if (data.images) {
          localStorage.setItem("defaultCat", JSON.stringify(data.images));
        }
      } catch (error) {
        console.log("Error fetching category data", error);
      }
      ...
      <div className="content px-2 py-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-x-4 gap-y-8">
          {catData
            ...
            : defaultCat?.images?.slice(0, 6).map((image, index) => (
                <div className="relative" key={index}>
                  <Link to={`/${defaultCat.category}`}>
                    <div className="overflow-hidden">
                      <div
                        className="relative w-full h-[275px] overflow-hidden bg-center bg-cover 
                      hover:bg-blend-multiply transition-all duration-300 ease-in-out hover:scale-105 transform group flex justify-center items-center"
                  ...
                  <p className="font-bold text-xs my-2">
                    ${defaultCat.listingPrice}
                  </p>
                  <div className="flex text-xs font-bold mb-2 space-x-2">
                    <p>{defaultCat.address.street}</p>
                    <p>{defaultCat.address.city}</p>
                    <p>{defaultCat.address.state}</p>
                    <p>{defaultCat.address.postalCode}</p>
                  </div>
                  <div className="flex text-xs font-light space-x-2 text-gray-500">
                    <p>{defaultCat.bedrooms} BEDS</p>
                    <p>{defaultCat.bathrooms} BATHS</p>
                    <p>{defaultCat.size.totalArea} SQ.FT</p>
                  </div>
                </div>
              ))}
        </div>
